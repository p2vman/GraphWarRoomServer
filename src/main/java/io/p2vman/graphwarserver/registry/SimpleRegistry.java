...
public class SimpleRegistry<T> implements MutableRegistry<T> {
    ...
    public SimpleRegistry(Identifier id) {
        this(id, new Object2ObjectOpenHashMap<>(), new ObjectArrayList<>());
    }

    public SimpleRegistry(String id) {
        this(new Identifier(id), new Object2ObjectOpenHashMap<>(), new ObjectArrayList<>());
    }
    ...
    public Registry<T> freeze() {
        return new SimpleRegistry<T>(id) {
            private final Map<Identifier, T> frozenRegistry = new Object2ObjectOpenHashMap<>(registry);
            ...
            @Override
            public Iterator<T> iterator() {
                return Collections.unmodifiableList(new ObjectArrayList<>(frozenRegistry.values())).iterator();
            }
